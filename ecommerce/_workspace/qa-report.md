...
## 실제 구동 E2E 검증 (5차) — 2026-07-28

1~4차는 전부 **MockMvc 통합 테스트 + 빌드**까지였다(PROGRESS.md "실제 구동 검증 미수행").
5차는 그 간극을 메운다: PostgreSQL·실제 HTTP 스택·실제 브라우저로 앱을 띄워 전 구간을 관통했다.

### 1. 기동 환경 (실제 확인)

| 구성 | 결과 |
|------|------|
| PostgreSQL 16 (docker compose) | healthy. `docker-compose.yml` 신규 추가 |
| 백엔드 `./gradlew bootRun` | **`Started EcommerceApplication in 2.303 seconds`** |
| `ddl-auto: update` 스키마 생성 | PostgreSQL에서 정상 — **H2 전용 DDL 의존 없음이 처음으로 확인됨** |
| `ProductSeeder` | 상품 12개 시드 (`totalElements=12`) |
| 프론트 `npm run dev` | `VITE v8.1.5 ready in 177 ms`, 포트 5173 |

### 2. API 레벨 E2E — `_workspace/e2e-smoke.sh`

계약 전 구간 **84개 항목 전부 통과 (실패 0)**. 이 스크립트를 저장소에 자산으로 남겼다.

통합 테스트가 구조적으로 덮지 못하는 층을 이번에 처음 검증했다.

- **커밋된 트랜잭션** — 통합 테스트는 롤백된다. 재고를 별도 HTTP 요청으로 재조회해
  `50 → 48 → 46` 으로 실제 영속됨을 확인.
- **CORS 프리플라이트** — `OPTIONS /api/cart` → 200, `Access-Control-Allow-Origin: http://localhost:5173`.
  MockMvc 슬라이스로는 검증 불가능한 경로였다.
- **PostgreSQL 방언** — 위 1항.
- **Jackson 3 날짜 직렬화** — `user.createdAt` 이 ISO 8601 **문자열**로 직렬화됨을 실 응답에서 확인.
- **`PageResponse` 계약 준수** — 응답에 `pageable`/`content` 누출 없음을 실 응답에서 확인.

핵심 정책 항목도 실구동으로 재확인:

| 항목 | 결과 |
|------|------|
| 결제 실패 = HTTP 200 + 본문 `status=FAILED` | 성립 |
| 빈 본문 `{}` → `simulateSuccess` 기본 SUCCESS | 성립 (PROGRESS.md가 경고한 회귀 지점) |
| FAILED 주문 재결제 시 Payment 행 재사용 | `payment.id` 동일 유지 |
| SUCCESS 주문 재결제 → 409 `ALREADY_PAID` | 성립 |
| 결제 실패 시 재고 미복원 (설계 결정) | 성립 |
| 소유권 검증 403 (장바구니 아이템 · 주문 조회 · 결제) | 성립 |

### 3. 브라우저 E2E (Playwright, 실제 렌더링)

회원가입 → 상품 목록 → 상품 상세 → 장바구니 담기 → 장바구니 → 체크아웃 → 주문 생성 →
**결제 실패** → **재결제 성공** → 주문 내역 → 로그아웃까지 관통.

- 전 구간 **콘솔 에러 0건**.
- 상품 이미지(`picsum.photos`) 실제 로드 확인 — 스크린샷 육안 확인.
- 주소창 직접 이동(`/cart`)으로 **새로고침 후 localStorage 토큰 복원** 확인.
- 로그아웃 후 `/orders` 접근 → `/login` 리다이렉트 (`RequireAuth` 정상).
- 비로그인 상태에서 `/products` 접근 가능 (계약상 인증 불필요) — 정상.
- 결제 실패 후 재시도 폼이 유지되고, 성공 후에는 폼이 사라지며 "결제가 완료된 주문입니다" 노출.
  BE의 `ALREADY_PAID` 정책이 UI에서도 일관되게 표현됨.

### 4. 발견된 불일치

| # | 내용 | 심각도 | 조치 |
|---|------|--------|------|
| 1 | 홈 화면(`HomePage.tsx`)에 Stage 1 시절 자리표시자 문구 잔존 — "장바구니 · 주문/결제 화면은 다음 단계에서 이 영역에 추가됩니다." Stage 3·4 완료 후에도 사용자에게 미완성이라 안내 | 중 | **해결.** 문구 제거 + 장바구니·주문 내역 진입점 추가. `tsc --noEmit`·`npm run build` 재통과 |

> 이 결함은 타입체크·빌드·MockMvc 어느 것으로도 잡히지 않는 종류다(문자열 상수는 컴파일러의 관심사가 아니다).
> 실제 화면을 열어야만 드러난다.

**관찰 사항(결함 아님):** 빈 장바구니 화면에서 "주문하기" 버튼이 `disabled` 상태로 계속 노출된다.
기능적으로 막혀 있으므로 결함은 아니나, 숨기는 편이 자연스럽다.

### 5. 게이트 판정

- **실제 구동 E2E 게이트: 통과(PASS)**
- 사유: (a) API 84항목 전부 통과, 미해결 0건. (b) 브라우저 전 구간 관통 성공, 콘솔 에러 0건.
  (c) 발견된 불일치 1건은 이번 세션에서 해결하고 빌드로 재검증. (d) PROGRESS.md가 유일한
  미검증 항목으로 남겨둔 "실제 구동"이 해소되었고, 재현 수단(`docker-compose.yml`,
  `e2e-smoke.sh`, `README.md`)을 저장소 자산으로 남겨 다음 세션이 같은 검증을 반복할 수 있다.

---

_Stage 4까지 4개 모듈(인증·상품·장바구니·주문/결제) 전체가 PASS. 계약 12개 엔드포인트 모두 검증 완료._
_5차에서 실제 구동(PostgreSQL + 브라우저) E2E까지 PASS — MVP 검증 완결._
