...
**최종 갱신:** 2026-07-28 (실제 구동 E2E 검증 완료 — **MVP 검증 완결**)
...
E2E:     실제 구동 검증 (PostgreSQL+브라우저)  ✅ 완료 (QA PASS)
```

> **MVP 완료.** 계약의 12개 엔드포인트(인증 2 · 상품 2 · 장바구니 4 · 주문 3 · 결제 1)가 전부 구현·검증되었고,
> 실제 구동(PostgreSQL + 브라우저) E2E까지 통과했다.
...
## 실제 구동 E2E 검증 요약 (완료, QA PASS) — 2026-07-28

Stage 4까지 남아 있던 유일한 미검증 항목("실제 구동 검증 미수행")을 해소했다.
상세는 `qa-report.md` "실제 구동 E2E 검증 (5차)".

**신규 저장소 자산 (다음 세션이 그대로 재현 가능):**
- `docker-compose.yml` — PostgreSQL 16. `application.yml` dev 데이터소스와 값 일치.
- `README.md` — 기동 순서·검증 명령·트러블슈팅. **JDK 25 필요**, `cleanTest test` 주의 포함.
- `_workspace/e2e-smoke.sh` — 계약 전 구간 **84개 항목** 검사. 백엔드 기동 상태에서 실행.

**실구동으로 처음 확인된 것 (통합 테스트가 구조적으로 못 보던 층):**
- **PostgreSQL 방언** — `ddl-auto: update` 스키마 생성 정상. H2 전용 DDL 의존 없음. 기동 2.3초.
- **커밋된 트랜잭션** — 재고가 `50 → 48 → 46` 으로 실제 영속됨(통합 테스트는 롤백되어 확인 불가했다).
- **CORS 프리플라이트** — `OPTIONS /api/cart` 200 + `Access-Control-Allow-Origin`. MockMvc 슬라이스로는 불가능.
- **브라우저 전 구간** — 회원가입→상품→장바구니→체크아웃→결제실패→재결제성공→주문내역→로그아웃, **콘솔 에러 0건**.
  새로고침 후 localStorage 토큰 복원, `RequireAuth` 리다이렉트, 비로그인 `/products` 접근 모두 정상.

**발견·해결한 결함 1건:**
- `HomePage.tsx`에 Stage 1 자리표시자 문구("…다음 단계에서 이 영역에 추가됩니다") 잔존 →
  제거하고 장바구니·주문 내역 진입점 추가. **타입체크·빌드·MockMvc 어느 것도 잡지 못하는 종류**였다
  (문자열 상수는 컴파일러의 관심사가 아니다). 실제 화면을 열어야만 드러난다.

**주의 (포트 변경 시):** 프론트는 `.env`의 `VITE_API_BASE_URL`로 백엔드를 **절대 URL 호출**한다(Vite 프록시 미사용).
프론트 포트를 5173/3000 밖으로 바꾸면 `SecurityConfig.corsConfigurationSource()`의 허용 오리진도 함께 고쳐야 한다.
...
- ~~**실제 구동 검증 미수행**~~ → **2026-07-28 해소.** 위 "실제 구동 E2E 검증 요약" 참조.

## 다음 작업 후보

1. **알려진 결함 3건 정리** — `OrderItem.productName` 스냅샷(계약 두 파일 정렬 포함),
   `PaymentService`의 `CANCELLED` 가드, `CartService.insertNewItem` 경합 복구 패턴.
   전부 "지금은 도달 불가하지만 다음 기능에서 터지는" 유형이다.
2. **Stage 5 기능 확장** — 주문 취소 API / 상품 검색 / 관리자 상품 CRUD.
   **선행 조건 주의:** 취소 API는 1번의 `CANCELLED` 가드 + 재고 복원 정책이,
   상품 CRUD는 `OrderItem.productName` 스냅샷이 먼저 정리되어야 한다.
3. **하네스 구성 정리** — `.claude/` 미커밋 상태. 에이전트 정의가 `ecommerce/.claude/`에 있어
   세션 루트(`harness/`)에서 `subagent_type` 자동 등록이 안 되는 문제도 그대로다.
4. **UX 다듬기(경미)** — 빈 장바구니에서 `disabled` "주문하기" 버튼 노출.
